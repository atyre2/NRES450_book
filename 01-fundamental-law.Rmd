...
1. **Deterministic processes** have only a single outcome given complete knowledge of the state of the system.
2. **Expected value** of a random variable is the mean value, or the 1st moment of the distribution.
3. **Instantaneous mortality rate** the rate at which mortality events occur when the interval of time is allowed to shrink towards zero.
4. **Solipsism** A metaphysical viewpoint that asserts reality is only the result of our imaginations.
